...
class DataProcessor:
    ...
    def prepare_training_data(
        df: pd.DataFrame,
        add_synthetic_trends: bool = True
    ) -> pd.DataFrame:
        """
        ...
        if add_synthetic_trends:
            ...
            logger.info(f"Original distribution - Declining: {declining_count}, Stable: {stable_count}")
            ...
            if declining_count < stable_count * 0.3:
                ...
                synthetic_indices = df[synthetic_declining].sample(
                    frac=0.3, 
                    random_state=42
                ).index
